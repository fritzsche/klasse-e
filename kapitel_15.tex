...
Die Kabelverluste entsprechen ganz genau dem Antennengewinn eines Dipols.
...
\begin{sol}{EG507}
 Die \SI{10}{\dB} Dämpfung entspricht einem Faktor von 10. D. h., \SI{100}{\watt} Ausgangsleistung werden zu \SI{10}{\watt} an der Antenne. Es handelt sich um einen Dipol, also müssen wir noch den Antennengewinn mit dem Faktor 1,64 berücksichtigen und bekommen also $\SI{10}{\watt} \cdot 1,64 = \SI{16,4}{\watt}$ heraus.
\end{sol}

\begin{sol}{EG508}
    Ähnlich zu Frage \qref{EG507}!
    Aus Kabelverlust und Antennengewinn haben wir \SI{3}{\dB}, also einen Faktor 2 laut der Umrechnungstabelle. Aus \SI{5}{\watt} werden mit dem Faktor 2 also \SI{10}{\watt}.
    Jetzt aber aufpassen: Der Antennengewinn war in dB (also in Bezug auf Dipol) angegeben. Um auf EIRP (Bezug Isotropenstrahler) zu kommen, müssen wir den Gewinn des Dipols mit dem Faktor 1,64 berücksichtigen. Das Ergebnis ist also auch $\SI{10}{\watt} \cdot 1,64 = \SI{16,4}{\watt}$.
\end{sol}

\begin{sol}{EG509}
    Aus Antennengewinn und Kabelverlust ergeben sich \SI{10}{\dB}, also Faktor 10 Gewinn. Aus \SI{0,6}{\watt} werden also \SI{6}{\watt}. Wir berücksichtigen aber noch den Dipol-Gewinnfaktor von 1,64 und erhalten: $\SI{6}{\watt} \cdot 1,64 = \SI{9,84}{\watt}$
\end{sol}
...
\begin{sol}{EG511}
    ...
    Rechnen wir von den maximal \SI{10}{\watt} EIRP zurück, ergibt sich $$10/3,1752 \approx \SI{3,1494}{\watt}$$, die wir maximal aussenden können.
...
\begin{ohmchapter}
 Es geht in diesem Kapitel mal wieder um die \SI{10}{\watt} EIRP-Strahlungsleistung.
\end{ohmchapter}
...
\begin{sol}{EK105}
Hier müssen wir nicht den Personensicherheitsabstand bestimmen. Es wurde aber mit der Formel für die Fernfeldberechnung ermittelt. Wir müssen lediglich überprüfen, ob der Abstand $d$ ausreicht.
Im Abschnitt Strahlungsleistung und Gewinn von Antennen finden wir unter Feldstärke im Fernfeld einer Antenne die Formel und auch die Angabe über den Abstand $$d > \frac{\lambda}{2\cdot \pi}$$
Für die Frequenz \SI{3,5}{\mega\hertz} ist 
$$ \lambda = \frac{300}{3,5} = \SI{85,71}{\meter}.$$

Wir setzen ein:
$$d > \frac{\lambda}{2\cdot \pi} = \frac{\SI{85,71}{\meter}}{2\cdot \pi} = \SI{13,64}{\meter}$$

Da die Rechnung erst ab \SI{13,64}{\meter} gültig ist und wir laut Frage \SI{3,65}{\meter} ermittelt haben, kann die Rechnung nicht verwendet werden. Wir müssen z.B. messen.
...
\begin{sol}{EK106}
    Wie in Frage \qref{EK105} überprüfen wir nur den Gültigkeitsbereich der Feldfeldberechnung. Wir wissen noch aus der vorherigen Frage, dass der Abstand im 80 m-Band näherungsweise \SI{13,64}{\meter} betrug. Wir finden eine sehr ähnliche Angabe in der richtigen Antwort (A). 
    Da $\lambda$ im Zähler ist, muss für 160 m etwas der doppelte Abstand gelten, auch das passt zu Antwort (A).
    Genau genommen sind die Angaben in Antwort (C) sogar noch kleiner. Diese Berechnungen wären also auch ungültig und Antwort (C) ist auch richtig. Aber die BNetzA lässt nur Antwort (A) zu.
\end{sol}


\begin{sol}{EK108}
    In der Formelsammlung finden wir 
    $$ E = \frac{\sqrt{\SI{30}{\ohm}\cdot P_{\text{EIRP}}}}{d}$$
    $$ d = \frac{\sqrt{\SI{30}{\ohm}\cdot P_{\text{EIRP}}}}{E}$$

    Wir haben \SI{7,5}{\dB} - \SI{1,5}{\dB} = \SI{6}{\dB}. Laut Tabelle in der Formelsammlung entspricht dies einem Faktor 4. Da die Antenne mit einem Gewinn in dBd angegeben ist, müssen wir noch den Dipol-Gewinnfaktor von 1,64 berücksichtigen: $4\cdot 1.64 = 6,56$.
    Mit der Senderausgangsleistung von \SI{100}{\watt}. Also $$P_{\text{EIRP}} = \SI{100}{\watt} \cdot 6,56 = \SI{656}{\watt}$$

    Wir setzen ein:
     $$ d = \frac{\sqrt{\SI{30}{\ohm}\cdot \SI{656}{\watt}}}{28 V/m} \approx \SI{5}{\meter}$$     
