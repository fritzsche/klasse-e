...
\begin{sol}{ED201}
In diesem Diagramm sehen wir, wie viel Leistung bei welcher Frequenz $f$ durchgelassen wird.
Wir sehen, dass wir unterhalb der mit gestrichelten Linien markierten Grenzfrequenz eine konstant hohe Leistung haben.
Über der \qq{Grenzfrequenz} fällt die Leistung schnell ab.
Da niedrige Frequenzen durchgelassen werden, aber hohe Frequenzen gefiltert werden, handelt es sich um einen Tiefpassfilter.
\end{sol}
...
\begin{ohmchapter}
    ...
    Zunächst erinnern wir uns an den Spannungsteiler.
    Zudem erinnern wir uns an den frequenzabhängigen Wechselstromwiderstand eines Kondensators $X_C$.
    $X_C$ ist bei niedrigen Frequenzen sehr hoch und niedrig für hohe Frequenzen. Merkhilfe: (Bei Frequenz \SI{0}{\hertz} leitet ein Kondensator nicht).

    Schauen wir uns einen sehr einfachen Filter an:
    \qgrafic[0.9]{ED208}

    Hätten wir in der Schaltung statt des Kondensators einen Widerstand, so hätten wir einen einfachen Spannungsteiler. Da wir hier aber einen Kondensator verwenden, der einen von der Frequenz abhängigen Widerstand hat.
   ...
   Die Filtercharakteristik eines solchen Hochpassfilters sieht also folgendermaßen aus:
  \qgrafic[0.9]{ED202}   


   Jetzt, da wir RC-Filter kennengelernt haben, erinnern wir uns an den Wechselstromwiderstand $X_L$ einer Spule. Der war genau umgekehrt zum Kondensator. Bei niedrigen Frequenzen leitet die Spule sehr gut und bei hohen Frequenzen haben wir mehr und mehr Widerstand.
   D. h., in unserem Spannungsteiler hätten wir auch einen Widerstand durch eine Spule ersetzen können und Hochpass und Tiefpass bauen können.
   Ersetzen wir einen der Widerstände durch einen Kondensator und den anderen durch eine Spule, so sind beide Widerstände unseres Spannungsteilers frequenzabhängig. Der Effekt wird verstärkt. Wir sprechen von einem LC-Filter.
...
Neben dem Parallelschwingkreis gibt es natürlich auch den \textbf{Serienschwingkreis}. 
 \qgrafic[0.9]{ED215} 
...
\begin{ohmchapter}
Im letzten Kapitel haben wir bereits den Parallelschwingkreis kennengelernt, in dem ein Kondensator und eine Spule in Resonanz gebracht werden. Solch ein Parallelschwingkreis ist das einfachste Beispiel von Schaltungen, die Schwingungen erzeugen. Zu so einem Schwingkreis sagen wir auch \textbf{Oszillator}. Im Fall des Parallelschwingkreises handelt es sich um einen \textbf{LC-Oszillator}.
...
\begin{sol}{EF302}
    Wir haben die Frequenzvervielfacher mit Faktoren $2\cdot 3 \cdot 2 = 12$. Wir rechnen rückwärts und teilen.
